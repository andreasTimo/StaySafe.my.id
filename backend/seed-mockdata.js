/**
 * StaySafe.my.id — seed-mockdata.js
 *
 * Script to seed the Firestore MongoDB-compatible database collection 'reports'
 * with 65 fresh, realistic Jakarta crime incident mock reports including human-readable addresses.
 *
 * Usage:
 *   MONGODB_URI="<YOUR_FIRESTORE_MONGODB_URI>" node backend/seed-mockdata.js
 */
...
async function seed() {
  console.log('Connecting to staysafe database...');
  try {
    const db = await getDb();
    const reportsCollection = db.collection('reports');

    console.log('Clearing existing documents in "reports" collection...');
    const deleteResult = await reportsCollection.deleteMany({});
    console.log(`Deleted ${deleteResult.deletedCount} existing documents.`);

    console.log(`Inserting ${MOCK_REPORTS.length} fresh mock reports into "reports" collection...`);
    const insertResult = await reportsCollection.insertMany(MOCK_REPORTS);
    console.log(`Successfully seeded ${insertResult.insertedCount} reports!`);
